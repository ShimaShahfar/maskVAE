...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netB = networks.define_B(
                netB_input_nc, opt.output_nc, 32, 3, 3, opt.norm, gpu_ids=self.gpu_ids
            )
    ...
    def encode_input(
        ...
        if self.opt.label_nc == 0:
            ...
            inter_label_1 = inter_label_1.scatter_(
                1, inter_label_map_1.data.long().cuda(), 1.0
            )
            inter_label_2 = torch.cuda.FloatTensor(torch.Size(oneHot_size)).zero_()
            inter_label_2 = inter_label_2.scatter_(
                1, inter_label_map_2.data.long().cuda(), 1.0
            )
    ...
    def forward(
        ...
        (
            ...
            inter_label_1, label, inter_label_2, image, label_ref, image_ref
        )
            ...
            for i in range(self.opt.num_D):
                for j in range(len(pred_fake[i]) - 1):
                    ...
                    loss_GB_GAN_Feat += (
                        D_weights
                        * feat_weights
                        * self.criterionFeat(pred_blend[i][j], pred_real[i][j].detach())
                        * self.opt.lambda_feat
                    )
        ...
        if not self.opt.no_vgg_loss:
            ...
            loss_GB_VGG += (
                self.criterionVGG(blend_image, real_image) * self.opt.lambda_feat
            )
    ...
    def inference(self, label, label_ref, image_ref):
        ...
        if torch.__version__.startswith("0.4"):
            with torch.no_grad():
                fake_image = self.netG.forward(
                    input_label, input_label_ref, real_image_ref
                )
        else:
